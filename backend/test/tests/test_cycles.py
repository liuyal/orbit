...
class TestOrbitTMCycles(OrbitTMBaseTest):
    ...
    def test_get_all_cycles_for_project(self, request):
        ...
        #         response = requests.post(f"{self.__class__.url}/tm/projects/{project_key}/test-cases/{test_case_key}/executions")
        #         assert response.status_code == 201
        #
        ...
        #
        #     response = requests.get(f"{self.__class__.url}/tm/projects/{project_key}/test-cases/{test_case_key}")
        #     assert response.status_code == 200
        #     assert response.json()["last_execution_key"] == f"{project_key}-E{j * te_count}"
        #
        # response = requests.get(f"{self.__class__.url}/tm/projects/{project_key}/executions")
        # assert response.status_code == 200
        # assert len(response.json()) == tc_count * te_count
        #
        # response = requests.get(f"{self.__class__.url}/tm/projects/{project_key}/cycles")
        # assert response.status_code == 200
        # assert len(response.json()) == 0
        #
        # for j in range(1, 7):
        #     response = requests.post(f"{self.__class__.url}/tm/projects/{project_key}/cycles")
        #     assert response.status_code == 201
        #
        # response = requests.get(f"{self.__class__.url}/tm/projects/{project_key}/cycles")
        # assert response.status_code == 200
        ...
        # response = requests.get(f"{self.__class__.url}/tm/cycles/{cycle_key}")
        # assert response.status_code == 200
        ...
        # response = requests.put(f"{self.__class__.url}/tm/cycles/{cycle_key}", json=payload)
        # assert response.status_code == 200
        # assert response.json()["test_cycle_key"] == cycle_key
        #
        # response = requests.get(f"{self.__class__.url}/tm/cycles/{cycle_key}")
        ...
        #
        # cycle_key = f"{project_key}-C{1}"
        # response = requests.get(f"{self.__class__.url}/tm/cycles/{cycle_key}/executions")
        # assert response.status_code == 200
        # assert len(response.json()) == 0
        #
        # for j in range(1, 7):
        ...
        #         response = requests.post(f"{self.__class__.url}/tm/cycles/{cycle_key}/executions?execution_key={execution_key}")
        #         assert response.status_code == 200
        #
        #     response = requests.get(f"{self.__class__.url}/tm/cycles/{cycle_key}/executions")
        #     assert response.status_code == 200
        ...
        # response = requests.delete(f"{self.__class__.url}/tm/cycles/{cycle_key}/executions/{execution_key}")
        # assert response.status_code == 200
        #
        # response = requests.get(f"{self.__class__.url}/tm/cycles/{cycle_key}/executions")
        # assert response.status_code == 200
        ...
        # cycle_key = f"{project_key}-C{6}"
        #
        # response = requests.delete(f"{self.__class__.url}/tm/cycles/{cycle_key}")
