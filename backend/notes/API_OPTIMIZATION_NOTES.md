...
- Project and test case fetched concurrently via `asyncio.gather` — eliminates the redundant second project lookup that
...
### `GET /tm/projects/{project_key}/test-cases/{test_case_key}/executions` →
`get_all_executions_by_test_case_key` ✅ Optimized

- Project and test case `find_one` checks run concurrently via `asyncio.gather`.

### `POST /tm/projects/{project_key}/test-cases/{test_case_key}/executions` →
`create_execution_by_test_case_key` ✅ Optimized
